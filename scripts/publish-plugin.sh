...
PLUGIN_PACKAGE_NAME="pulumi-resource-gcp-${VERSION}-$(go env GOOS)-$(go env GOARCH).tar.gz"
...
(cd "${ROOT}/provider" && go build \
   -ldflags "-X github.com/pulumi/pulumi-gcp/provider/v3/pkg/version.Version=${VERSION}" \
...
CREDS_JSON=$(aws sts assume-role \
                 ...
                 --role-session-name "upload-plugin-pulumi-resource-gcp" \
