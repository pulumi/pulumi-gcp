...
// * `binaryauthorization.AttestorIamBinding`: Authoritative for a given role. Updates the IAM policy to grant a role to a list of members. Other roles within the IAM policy for the attestor are preserved. Members added outside of Terraform for the same role will be detected as drift and removed on the next `pulumi up`.
// * `binaryauthorization.AttestorIamMember`: Non-authoritative. Updates the IAM policy to grant a role to a new member. Other members for the role for the attestor are preserved. Members added outside of Terraform will **not** be detected as drift.
//
...
// import (
//
//	"github.com/pulumi/pulumi-gcp/sdk/v10/go/gcp/binaryauthorization"
