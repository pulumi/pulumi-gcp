...
// * `secretmanager.RegionalSecretIamMember`: Non-authoritative. Updates the IAM policy to grant a role to a new member. Other members for the same role and condition combination for the regionalsecret are preserved. Members added outside of Terraform will **not** be detected as drift.
//
...
//
// > **Note:** `secretmanager.RegionalSecretIamBinding` resources **can be** used in conjunction with `secretmanager.RegionalSecretIamMember` resources **only if** they do not grant privilege to the same role and condition combination.
//
...
type RegionalSecretIamBinding struct {
	...
	// `secretmanager.RegionalSecretIamBinding` can be used per role and condition combination. Multiple bindings for the same role are allowed if each has a different `condition` block (or one has no condition). Note that custom roles must be of the format
