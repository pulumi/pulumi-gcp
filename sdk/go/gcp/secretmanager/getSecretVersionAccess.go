...
func GetSecretVersionAccess(ctx *pulumi.Context, args *GetSecretVersionAccessArgs, opts ...pulumi.InvokeOption) (*GetSecretVersionAccessResult, error) {
	...
	if optsErr != nil {
		return &GetSecretVersionAccessResult{}, optsErr
	}
	if len(invokeOpts.DependsOn) > 0 {
		return &GetSecretVersionAccessResult{}, errors.New("DependsOn is not supported for direct form invoke GetSecretVersionAccess, use GetSecretVersionAccessOutput instead")
	}
	if len(invokeOpts.DependsOnInputs) > 0 {
		return &GetSecretVersionAccessResult{}, errors.New("DependsOnInputs is not supported for direct form invoke GetSecretVersionAccess, use GetSecretVersionAccessOutput instead")
	}
...
func GetSecretVersionAccessOutput(ctx *pulumi.Context, args GetSecretVersionAccessOutputArgs, opts ...pulumi.InvokeOption) GetSecretVersionAccessResultOutput {
	return pulumi.ToOutputWithContext(ctx.Context(), args).
		ApplyT(func(v interface{}) (GetSecretVersionAccessResultOutput, error) {
			...
			opts = internal.PkgInvokeDefaultOpts(opts)
			var rv GetSecretVersionAccessResult
			secret, deps, err := ctx.InvokePackageRawWithDeps("gcp:secretmanager/getSecretVersionAccess:getSecretVersionAccess", args, &rv, "", opts...)
			if err != nil {
				return GetSecretVersionAccessResultOutput{}, err
			}

			output := pulumi.ToOutput(rv).(GetSecretVersionAccessResultOutput)
			output = pulumi.OutputWithDependencies(ctx.Context(), output, deps...).(GetSecretVersionAccessResultOutput)
			if secret {
				return pulumi.ToSecret(output).(GetSecretVersionAccessResultOutput), nil
			}
			return output, nil
