// *** WARNING: this file was generated by pulumi-language-java. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.gcp.ces.outputs;

import com.pulumi.core.annotations.CustomType;
import java.lang.String;
import java.util.Objects;
import java.util.Optional;
import javax.annotation.Nullable;

@CustomType
public final class ToolWidgetToolDataMappingPythonFunction {
    /**
     * @return (Output)
     * The description of the Python function, parsed from the python code&#39;s
     * docstring.
     * 
     */
    private @Nullable String description;
    /**
     * @return Optional. The name of the Python function to execute. Must match a Python function
     * name defined in the python code. Case sensitive. If the name is not
     * provided, the first function defined in the python code will be used.
     * 
     */
    private @Nullable String name;
    /**
     * @return Optional. The Python code to execute for the tool.
     * 
     */
    private @Nullable String pythonCode;

    private ToolWidgetToolDataMappingPythonFunction() {}
    /**
     * @return (Output)
     * The description of the Python function, parsed from the python code&#39;s
     * docstring.
     * 
     */
    public Optional<String> description() {
        return Optional.ofNullable(this.description);
    }
    /**
     * @return Optional. The name of the Python function to execute. Must match a Python function
     * name defined in the python code. Case sensitive. If the name is not
     * provided, the first function defined in the python code will be used.
     * 
     */
    public Optional<String> name() {
        return Optional.ofNullable(this.name);
    }
    /**
     * @return Optional. The Python code to execute for the tool.
     * 
     */
    public Optional<String> pythonCode() {
        return Optional.ofNullable(this.pythonCode);
    }

    public static Builder builder() {
        return new Builder();
    }

    public static Builder builder(ToolWidgetToolDataMappingPythonFunction defaults) {
        return new Builder(defaults);
    }
    @CustomType.Builder
    public static final class Builder {
        private @Nullable String description;
        private @Nullable String name;
        private @Nullable String pythonCode;
        public Builder() {}
        public Builder(ToolWidgetToolDataMappingPythonFunction defaults) {
    	      Objects.requireNonNull(defaults);
    	      this.description = defaults.description;
    	      this.name = defaults.name;
    	      this.pythonCode = defaults.pythonCode;
        }

        @CustomType.Setter
        public Builder description(@Nullable String description) {

            this.description = description;
            return this;
        }
        @CustomType.Setter
        public Builder name(@Nullable String name) {

            this.name = name;
            return this;
        }
        @CustomType.Setter
        public Builder pythonCode(@Nullable String pythonCode) {

            this.pythonCode = pythonCode;
            return this;
        }
        public ToolWidgetToolDataMappingPythonFunction build() {
            final var _resultValue = new ToolWidgetToolDataMappingPythonFunction();
            _resultValue.description = description;
            _resultValue.name = name;
            _resultValue.pythonCode = pythonCode;
            return _resultValue;
        }
    }
}
