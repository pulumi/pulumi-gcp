// *** WARNING: this file was generated by pulumi-language-java. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.gcp.dataproc.outputs;

import com.pulumi.core.annotations.CustomType;
import java.lang.Integer;
import java.lang.String;
import java.util.Objects;
import java.util.Optional;
import javax.annotation.Nullable;

@CustomType
public final class ClusterClusterConfigWorkerConfigDiskConfigAttachedDiskConfig {
    /**
     * @return Size of the attached disk, specified in GB.
     * 
     */
    private @Nullable Integer diskSizeGb;
    /**
     * @return The disk type of the attached disk. Such as &#34;pd-ssd&#34; or &#34;pd-standard&#34;.
     * 
     */
    private @Nullable String diskType;
    /**
     * @return Indicates how many IOPS to provision for the disk.
     * 
     */
    private @Nullable Integer provisionedIops;
    /**
     * @return Indicates how much throughput to provision for the disk.
     * 
     */
    private @Nullable Integer provisionedThroughput;

    private ClusterClusterConfigWorkerConfigDiskConfigAttachedDiskConfig() {}
    /**
     * @return Size of the attached disk, specified in GB.
     * 
     */
    public Optional<Integer> diskSizeGb() {
        return Optional.ofNullable(this.diskSizeGb);
    }
    /**
     * @return The disk type of the attached disk. Such as &#34;pd-ssd&#34; or &#34;pd-standard&#34;.
     * 
     */
    public Optional<String> diskType() {
        return Optional.ofNullable(this.diskType);
    }
    /**
     * @return Indicates how many IOPS to provision for the disk.
     * 
     */
    public Optional<Integer> provisionedIops() {
        return Optional.ofNullable(this.provisionedIops);
    }
    /**
     * @return Indicates how much throughput to provision for the disk.
     * 
     */
    public Optional<Integer> provisionedThroughput() {
        return Optional.ofNullable(this.provisionedThroughput);
    }

    public static Builder builder() {
        return new Builder();
    }

    public static Builder builder(ClusterClusterConfigWorkerConfigDiskConfigAttachedDiskConfig defaults) {
        return new Builder(defaults);
    }
    @CustomType.Builder
    public static final class Builder {
        private @Nullable Integer diskSizeGb;
        private @Nullable String diskType;
        private @Nullable Integer provisionedIops;
        private @Nullable Integer provisionedThroughput;
        public Builder() {}
        public Builder(ClusterClusterConfigWorkerConfigDiskConfigAttachedDiskConfig defaults) {
    	      Objects.requireNonNull(defaults);
    	      this.diskSizeGb = defaults.diskSizeGb;
    	      this.diskType = defaults.diskType;
    	      this.provisionedIops = defaults.provisionedIops;
    	      this.provisionedThroughput = defaults.provisionedThroughput;
        }

        @CustomType.Setter
        public Builder diskSizeGb(@Nullable Integer diskSizeGb) {

            this.diskSizeGb = diskSizeGb;
            return this;
        }
        @CustomType.Setter
        public Builder diskType(@Nullable String diskType) {

            this.diskType = diskType;
            return this;
        }
        @CustomType.Setter
        public Builder provisionedIops(@Nullable Integer provisionedIops) {

            this.provisionedIops = provisionedIops;
            return this;
        }
        @CustomType.Setter
        public Builder provisionedThroughput(@Nullable Integer provisionedThroughput) {

            this.provisionedThroughput = provisionedThroughput;
            return this;
        }
        public ClusterClusterConfigWorkerConfigDiskConfigAttachedDiskConfig build() {
            final var _resultValue = new ClusterClusterConfigWorkerConfigDiskConfigAttachedDiskConfig();
            _resultValue.diskSizeGb = diskSizeGb;
            _resultValue.diskType = diskType;
            _resultValue.provisionedIops = provisionedIops;
            _resultValue.provisionedThroughput = provisionedThroughput;
            return _resultValue;
        }
    }
}
