// *** WARNING: this file was generated by pulumi-language-java. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.gcp.ces.outputs;

import com.pulumi.core.annotations.CustomType;
import java.lang.String;
import java.util.Objects;
import java.util.Optional;
import javax.annotation.Nullable;

@CustomType
public final class AppVersionSnapshotAppDefaultChannelProfileWebWidgetConfig {
    /**
     * @return (Output)
     * The modality of the web widget.
     * Possible values:
     * UNKNOWN_MODALITY
     * CHAT_AND_VOICE
     * VOICE_ONLY
     * CHAT_ONLY
     * 
     */
    private @Nullable String modality;
    /**
     * @return (Output)
     * The theme of the web widget.
     * Possible values:
     * UNKNOWN_THEME
     * LIGHT
     * DARK
     * 
     */
    private @Nullable String theme;
    /**
     * @return (Output)
     * The title of the web widget.
     * 
     */
    private @Nullable String webWidgetTitle;

    private AppVersionSnapshotAppDefaultChannelProfileWebWidgetConfig() {}
    /**
     * @return (Output)
     * The modality of the web widget.
     * Possible values:
     * UNKNOWN_MODALITY
     * CHAT_AND_VOICE
     * VOICE_ONLY
     * CHAT_ONLY
     * 
     */
    public Optional<String> modality() {
        return Optional.ofNullable(this.modality);
    }
    /**
     * @return (Output)
     * The theme of the web widget.
     * Possible values:
     * UNKNOWN_THEME
     * LIGHT
     * DARK
     * 
     */
    public Optional<String> theme() {
        return Optional.ofNullable(this.theme);
    }
    /**
     * @return (Output)
     * The title of the web widget.
     * 
     */
    public Optional<String> webWidgetTitle() {
        return Optional.ofNullable(this.webWidgetTitle);
    }

    public static Builder builder() {
        return new Builder();
    }

    public static Builder builder(AppVersionSnapshotAppDefaultChannelProfileWebWidgetConfig defaults) {
        return new Builder(defaults);
    }
    @CustomType.Builder
    public static final class Builder {
        private @Nullable String modality;
        private @Nullable String theme;
        private @Nullable String webWidgetTitle;
        public Builder() {}
        public Builder(AppVersionSnapshotAppDefaultChannelProfileWebWidgetConfig defaults) {
    	      Objects.requireNonNull(defaults);
    	      this.modality = defaults.modality;
    	      this.theme = defaults.theme;
    	      this.webWidgetTitle = defaults.webWidgetTitle;
        }

        @CustomType.Setter
        public Builder modality(@Nullable String modality) {

            this.modality = modality;
            return this;
        }
        @CustomType.Setter
        public Builder theme(@Nullable String theme) {

            this.theme = theme;
            return this;
        }
        @CustomType.Setter
        public Builder webWidgetTitle(@Nullable String webWidgetTitle) {

            this.webWidgetTitle = webWidgetTitle;
            return this;
        }
        public AppVersionSnapshotAppDefaultChannelProfileWebWidgetConfig build() {
            final var _resultValue = new AppVersionSnapshotAppDefaultChannelProfileWebWidgetConfig();
            _resultValue.modality = modality;
            _resultValue.theme = theme;
            _resultValue.webWidgetTitle = webWidgetTitle;
            return _resultValue;
        }
    }
}
