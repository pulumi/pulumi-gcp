 ...
 * ```sh
 * $ terraform import google_iap_web_type_compute_iam_member.editor &#34;projects/{{project}}/iap_web/compute roles/iap.httpsResourceAccessor user:jane{@literal @}example.com&#34;
 * ```
 * 
...
public class WebTypeComputeIamMember extends com.pulumi.resources.CustomResource {
     ...
     * `gcp.iap.WebTypeComputeIamBinding` can be used per role and condition combination. Multiple bindings for the same role are allowed if each has a different `condition` block (or one has no condition). Note that custom roles must be of the format
