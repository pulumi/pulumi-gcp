// *** WARNING: this file was generated by pulumi-language-java. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.gcp.backupdisasterrecovery.inputs;

import com.pulumi.core.Output;
import com.pulumi.core.annotations.Import;
import java.lang.Boolean;
import java.lang.Integer;
import java.lang.String;
import java.util.Objects;
import java.util.Optional;
import javax.annotation.Nullable;


public final class RestoreWorkloadComputeInstanceRestorePropertiesNetworkInterfaceAccessConfigArgs extends com.pulumi.resources.ResourceArgs {

    public static final RestoreWorkloadComputeInstanceRestorePropertiesNetworkInterfaceAccessConfigArgs Empty = new RestoreWorkloadComputeInstanceRestorePropertiesNetworkInterfaceAccessConfigArgs();

    /**
     * (Optional)
     * 
     */
    @Import(name="externalIp")
    private @Nullable Output<String> externalIp;

    /**
     * @return (Optional)
     * 
     */
    public Optional<Output<String>> externalIp() {
        return Optional.ofNullable(this.externalIp);
    }

    /**
     * (Optional)
     * 
     */
    @Import(name="externalIpv6")
    private @Nullable Output<String> externalIpv6;

    /**
     * @return (Optional)
     * 
     */
    public Optional<Output<String>> externalIpv6() {
        return Optional.ofNullable(this.externalIpv6);
    }

    /**
     * (Optional)
     * 
     */
    @Import(name="externalIpv6PrefixLength")
    private @Nullable Output<Integer> externalIpv6PrefixLength;

    /**
     * @return (Optional)
     * 
     */
    public Optional<Output<Integer>> externalIpv6PrefixLength() {
        return Optional.ofNullable(this.externalIpv6PrefixLength);
    }

    /**
     * Optional. The name of this access configuration.
     * 
     */
    @Import(name="name")
    private @Nullable Output<String> name;

    /**
     * @return Optional. The name of this access configuration.
     * 
     */
    public Optional<Output<String>> name() {
        return Optional.ofNullable(this.name);
    }

    /**
     * Possible values are: `NETWORK_TIER_UNSPECIFIED`, `PREMIUM`, `STANDARD`.
     * 
     */
    @Import(name="networkTier")
    private @Nullable Output<String> networkTier;

    /**
     * @return Possible values are: `NETWORK_TIER_UNSPECIFIED`, `PREMIUM`, `STANDARD`.
     * 
     */
    public Optional<Output<String>> networkTier() {
        return Optional.ofNullable(this.networkTier);
    }

    /**
     * (Optional)
     * 
     */
    @Import(name="publicPtrDomainName")
    private @Nullable Output<String> publicPtrDomainName;

    /**
     * @return (Optional)
     * 
     */
    public Optional<Output<String>> publicPtrDomainName() {
        return Optional.ofNullable(this.publicPtrDomainName);
    }

    /**
     * (Optional)
     * 
     */
    @Import(name="setPublicPtr")
    private @Nullable Output<Boolean> setPublicPtr;

    /**
     * @return (Optional)
     * 
     */
    public Optional<Output<Boolean>> setPublicPtr() {
        return Optional.ofNullable(this.setPublicPtr);
    }

    /**
     * Optional. The type of configuration.
     * Possible values are: `ACCESS_TYPE_UNSPECIFIED`, `ONE_TO_ONE_NAT`, `DIRECT_IPV6`.
     * 
     */
    @Import(name="type")
    private @Nullable Output<String> type;

    /**
     * @return Optional. The type of configuration.
     * Possible values are: `ACCESS_TYPE_UNSPECIFIED`, `ONE_TO_ONE_NAT`, `DIRECT_IPV6`.
     * 
     */
    public Optional<Output<String>> type() {
        return Optional.ofNullable(this.type);
    }

    private RestoreWorkloadComputeInstanceRestorePropertiesNetworkInterfaceAccessConfigArgs() {}

    private RestoreWorkloadComputeInstanceRestorePropertiesNetworkInterfaceAccessConfigArgs(RestoreWorkloadComputeInstanceRestorePropertiesNetworkInterfaceAccessConfigArgs $) {
        this.externalIp = $.externalIp;
        this.externalIpv6 = $.externalIpv6;
        this.externalIpv6PrefixLength = $.externalIpv6PrefixLength;
        this.name = $.name;
        this.networkTier = $.networkTier;
        this.publicPtrDomainName = $.publicPtrDomainName;
        this.setPublicPtr = $.setPublicPtr;
        this.type = $.type;
    }

    public static Builder builder() {
        return new Builder();
    }
    public static Builder builder(RestoreWorkloadComputeInstanceRestorePropertiesNetworkInterfaceAccessConfigArgs defaults) {
        return new Builder(defaults);
    }

    public static final class Builder {
        private RestoreWorkloadComputeInstanceRestorePropertiesNetworkInterfaceAccessConfigArgs $;

        public Builder() {
            $ = new RestoreWorkloadComputeInstanceRestorePropertiesNetworkInterfaceAccessConfigArgs();
        }

        public Builder(RestoreWorkloadComputeInstanceRestorePropertiesNetworkInterfaceAccessConfigArgs defaults) {
            $ = new RestoreWorkloadComputeInstanceRestorePropertiesNetworkInterfaceAccessConfigArgs(Objects.requireNonNull(defaults));
        }

        /**
         * @param externalIp (Optional)
         * 
         * @return builder
         * 
         */
        public Builder externalIp(@Nullable Output<String> externalIp) {
            $.externalIp = externalIp;
            return this;
        }

        /**
         * @param externalIp (Optional)
         * 
         * @return builder
         * 
         */
        public Builder externalIp(String externalIp) {
            return externalIp(Output.of(externalIp));
        }

        /**
         * @param externalIpv6 (Optional)
         * 
         * @return builder
         * 
         */
        public Builder externalIpv6(@Nullable Output<String> externalIpv6) {
            $.externalIpv6 = externalIpv6;
            return this;
        }

        /**
         * @param externalIpv6 (Optional)
         * 
         * @return builder
         * 
         */
        public Builder externalIpv6(String externalIpv6) {
            return externalIpv6(Output.of(externalIpv6));
        }

        /**
         * @param externalIpv6PrefixLength (Optional)
         * 
         * @return builder
         * 
         */
        public Builder externalIpv6PrefixLength(@Nullable Output<Integer> externalIpv6PrefixLength) {
            $.externalIpv6PrefixLength = externalIpv6PrefixLength;
            return this;
        }

        /**
         * @param externalIpv6PrefixLength (Optional)
         * 
         * @return builder
         * 
         */
        public Builder externalIpv6PrefixLength(Integer externalIpv6PrefixLength) {
            return externalIpv6PrefixLength(Output.of(externalIpv6PrefixLength));
        }

        /**
         * @param name Optional. The name of this access configuration.
         * 
         * @return builder
         * 
         */
        public Builder name(@Nullable Output<String> name) {
            $.name = name;
            return this;
        }

        /**
         * @param name Optional. The name of this access configuration.
         * 
         * @return builder
         * 
         */
        public Builder name(String name) {
            return name(Output.of(name));
        }

        /**
         * @param networkTier Possible values are: `NETWORK_TIER_UNSPECIFIED`, `PREMIUM`, `STANDARD`.
         * 
         * @return builder
         * 
         */
        public Builder networkTier(@Nullable Output<String> networkTier) {
            $.networkTier = networkTier;
            return this;
        }

        /**
         * @param networkTier Possible values are: `NETWORK_TIER_UNSPECIFIED`, `PREMIUM`, `STANDARD`.
         * 
         * @return builder
         * 
         */
        public Builder networkTier(String networkTier) {
            return networkTier(Output.of(networkTier));
        }

        /**
         * @param publicPtrDomainName (Optional)
         * 
         * @return builder
         * 
         */
        public Builder publicPtrDomainName(@Nullable Output<String> publicPtrDomainName) {
            $.publicPtrDomainName = publicPtrDomainName;
            return this;
        }

        /**
         * @param publicPtrDomainName (Optional)
         * 
         * @return builder
         * 
         */
        public Builder publicPtrDomainName(String publicPtrDomainName) {
            return publicPtrDomainName(Output.of(publicPtrDomainName));
        }

        /**
         * @param setPublicPtr (Optional)
         * 
         * @return builder
         * 
         */
        public Builder setPublicPtr(@Nullable Output<Boolean> setPublicPtr) {
            $.setPublicPtr = setPublicPtr;
            return this;
        }

        /**
         * @param setPublicPtr (Optional)
         * 
         * @return builder
         * 
         */
        public Builder setPublicPtr(Boolean setPublicPtr) {
            return setPublicPtr(Output.of(setPublicPtr));
        }

        /**
         * @param type Optional. The type of configuration.
         * Possible values are: `ACCESS_TYPE_UNSPECIFIED`, `ONE_TO_ONE_NAT`, `DIRECT_IPV6`.
         * 
         * @return builder
         * 
         */
        public Builder type(@Nullable Output<String> type) {
            $.type = type;
            return this;
        }

        /**
         * @param type Optional. The type of configuration.
         * Possible values are: `ACCESS_TYPE_UNSPECIFIED`, `ONE_TO_ONE_NAT`, `DIRECT_IPV6`.
         * 
         * @return builder
         * 
         */
        public Builder type(String type) {
            return type(Output.of(type));
        }

        public RestoreWorkloadComputeInstanceRestorePropertiesNetworkInterfaceAccessConfigArgs build() {
            return $;
        }
    }

}
