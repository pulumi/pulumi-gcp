...
class ServicePerimeter(pulumi.CustomResource):
    ...
    def _internal_init(__self__,
        ...
        if opts.id is None:
            ...
            if spec is not None and not isinstance(spec, ServicePerimeterSpecArgs):
                spec = spec or {}
                def _setter(key, value):
                    spec[key] = value
                ServicePerimeterSpecArgs._configure(_setter, **spec)
            __props__.__dict__["spec"] = spec
            if status is not None and not isinstance(status, ServicePerimeterStatusArgs):
                status = status or {}
                def _setter(key, value):
                    status[key] = value
                ServicePerimeterStatusArgs._configure(_setter, **status)
