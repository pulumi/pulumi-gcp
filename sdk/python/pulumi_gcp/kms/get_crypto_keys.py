...
def get_crypto_keys(filter: Optional[_builtins.str] = None,
    ...
    :param _builtins.str filter: The filter argument is used to add a filter query parameter that limits which keys are retrieved by the data source: ?filter={{filter}}. When no value is provided there is no filtering.
