...
def get_crypto_keys(filter: Optional[builtins.str] = None,
    ...
    :param builtins.str filter: The filter argument is used to add a filter query parameter that limits which keys are retrieved by the data source: ?filter={{filter}}. When no value is provided there is no filtering.
...
def get_crypto_keys_output(filter: Optional[pulumi.Input[Optional[builtins.str]]] = None,
