...
def get_secrets(filter: Optional[_builtins.str] = None,
    ...
    :param _builtins.str filter: Filter string, adhering to the rules in [List-operation filtering](https://cloud.google.com/secret-manager/docs/filtering). List only secrets matching the filter. If filter is empty, all secrets are listed.
...
def get_secrets_output(filter: pulumi.Input[Optional[Optional[_builtins.str]]] = None,
