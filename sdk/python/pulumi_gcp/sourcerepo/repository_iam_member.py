...
@pulumi.type_token("gcp:sourcerepo/repositoryIamMember:RepositoryIamMember")
class RepositoryIamMember(pulumi.CustomResource):
    ...
    def __init__(__self__,
        ...
        * `sourcerepo.RepositoryIamBinding`: Authoritative for a given role. Updates the IAM policy to grant a role to a list of members. Other roles within the IAM policy for the repository are preserved. Members added outside of Terraform for the same role will be detected as drift and removed on the next `pulumi up`.
        * `sourcerepo.RepositoryIamMember`: Non-authoritative. Updates the IAM policy to grant a role to a new member. Other members for the role for the repository are preserved. Members added outside of Terraform will **not** be detected as drift.
