...
def get_web_iam_policy(project: Optional[str] = None,
                       opts: Optional[pulumi.InvokeOptions] = None) -> AwaitableGetWebIamPolicyResult:
    """
    ...
    policy = gcp.iap.get_web_iam_policy(project=%!v(PANIC=Format method: runtime error: invalid memory address or nil pointer dereference))
    ```
    ...
    __ret__ = pulumi.runtime.invoke('gcp:iap/getWebIamPolicy:getWebIamPolicy', __args__, opts=opts, typ=GetWebIamPolicyResult).value

    return AwaitableGetWebIamPolicyResult(
        etag=__ret__.etag,
        id=__ret__.id,
