...
calass Service(pulumi.CustomResource):
    ...
    def __init__(__self__,
        ...
        * How-to Guides
        ...
        Service can be imported using any of these accepted formats* `locations/{{location}}/namespaces/{{project}}/services/{{name}}` * `{{location}}/{{project}}/{{name}}` * `{{location}}/{{name}}` In Terraform v1.5.0 and later, use an [`import` block](https://developer.hashicorp.com/terraform/language/import) to import Service using one of the formats above. For exampletf import {
        ...
        ```sh
         $ pulumi import gcp:cloudrun/service:Service When using the [`terraform import` command](https://developer.hashicorp.com/terraform/cli/commands/import), Service can be imported using one of the formats above. For example
        ```
