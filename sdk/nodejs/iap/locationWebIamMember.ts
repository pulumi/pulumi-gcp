...
export class LocationWebIamMember extends pulumi.CustomResource {
     ...
     * `gcp.iap.LocationWebIamBinding` can be used per role and condition combination. Multiple bindings for the same role are allowed if each has a different `condition` block (or one has no condition). Note that custom roles must be of the format
...
export interface LocationWebIamMemberState {
     ...
     * Structure is documented below.
     */
    condition?: pulumi.Input<inputs.iap.LocationWebIamMemberCondition | undefined>;
    /**
     * (Computed) The etag of the IAM policy.
     */
