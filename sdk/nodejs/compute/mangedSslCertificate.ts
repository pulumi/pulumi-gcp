 ...
 *
 * ```sh
 *  $ pulumi import gcp:compute/mangedSslCertificate:MangedSslCertificate When using the [`terraform import` command](https://developer.hashicorp.com/terraform/cli/commands/import), ManagedSslCertificate can be imported using one of the formats above. For example
 * ```
 *
 * ```sh
